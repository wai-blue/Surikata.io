<?php

$dictionary = [
  "Company Info" => "Informácie o spoločnosti",
  "General" => "Všeobecné",
  "Slogan" => "Slogan",
  "Headquarter" => "Centrála",
  "Street, 1st line" => "Ulica, 1. riadok",
  "Street, 2st line" => "Ulica, ľ. riadok",
  "ZIP" => "PČS",
  "City" => "Mesto",
  "Region" => "Okres",
  "Country" => "Štát",
  "Contact information" => "Kontaktné infomácie",
  "Contact phone number" => "Kontakné mobilné číslo",
  "Contact email" => "Kontaktný e-mail",
  "Social networks" => "Sociálne siete",
  "Facebook page URL" => "Odkaz na facebook-ovú stránku",
  "Twitter URL" => "Odkaz na Twitter",
  "Instagram profile URL" => "Odkaz na instagramový profil",
  "YouTube channel URL" => "Odkaz na Youtube kanál",
];